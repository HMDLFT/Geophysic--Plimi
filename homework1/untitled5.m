...
model.vel =ones(Nz,Nx)*3000;%aa;%load('modaa');%ones(Nz,Nx)*1000;%;  % Higher velocity area

% Define a region of lower velocity (e.g., a convex lens)
center_x = 500;
center_z = 150;
focal_length = 150;  % Adjust the focal length to decrease the thickness of the lens
...
source.z    = [1:299];
Nsources    = numel(source.z);

source.x    = ones(1,Nsources) * 150; 
source.f0   = ones(1,Nsources) * 30; 
source.t0   = ones(1,Nsources) * 0.04
source.amp  = 10*tukeywin(Nsources,.5);
...
model.recx  = [570,570,570];
Nreceivers  = numel(model.recx);

model.recz  = [100,150,200];
model.dtrec = 0.004;
