...
[X, Z] = meshgrid(x, z);
vel = 1500 * ones(size(X));

% Add random variations on one side of the source
noisy_zone = X >= 1000;
vel(noisy_zone) = vel(noisy_zone) + 50 * randn(size(vel(noisy_zone)));
...
source.amp = 1;   % Amplitude of the source
...
model.recx = [600, 600, 600, 600; 1400, 1400, 1400,1400];
model.recz = [200, 400, 600,800; 200, 400, 600,800];
