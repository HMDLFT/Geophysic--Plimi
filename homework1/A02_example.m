...
model.recx  = [300, 700, 300, 700, 300, 700,300, 700, 300, 700];
model.recz  = [60,60,180,180,120, 120, 150, 150, 90, 90];  % ... a trick to have same nr elements  of recx
model.dtrec = 0.004;
% % model.recz  = [5:50:(model.z(end)-5)];
% % Nreceivers  = numel(model.recz);
% % 
% % model.recx  = ones(1,Nreceivers) * model.x(400)-10;
% % model.dtrec = 0.004;
...
source.x    = [mean(model.x)];
source.z    = [mean(model.z)]; 
source.f0   = [25 ];
source.t0   = [0.04  ];
source.amp  = [1 ];
